...
async function getFolderId(instance, folderName, parentId = "root", isShared = false, insertIfNotFound = false) {
  ...
  const result = await instance.get("/drive/v2/files", {
    params: {
      q: isShared
          ? `title = '${folderName.replace(/\'/g, "\\'")}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false and sharedWithMe = true`
          : `'${parentId}' in parents and title = '${folderName.replace(/\'/g, "\\'")}' and mimeType = 'application/vnd.google-apps.folder' and trashed = false`,
...
async function getFileId(instance, fileName, parentId = "root", isShared = false, errorOutIfExists = false) {
  ...
  const result = await instance.get("/drive/v2/files", {
    params: {
      q: isShared 
        ? `title = '${fileName.replace(/\'/g, "\\'")}' and sharedWithMe = true and trashed = false`
        : `'${parentId}' in parents and title = '${fileName.replace(/\'/g, "\\'")}' and trashed = false`,
      fields: `items(id, title)`
    }
  })
...
class GoogleDriveDataProvider extends Provider {
  ...
  async read(body, headers, params, queries) {
    ...
    if (diskPath(params["folderPath"]) === "/Shared") {
      ...
      q = `title = '${fileName.replace(/\'/g, "\\'")}' and trashed = false and sharedWithMe = true`
    } else {
      // Else just do a normal get
      q = `title = '${fileName.replace(/\'/g, "\\'")}' and '${folderId}' in parents and trashed = false`
    }
