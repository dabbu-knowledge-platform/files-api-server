...
beforeAll(async () => {
	...
	const serverResponse = await axios({
		...
		data: `client_id=${encodeURIComponent(
			process.env.DABBU_TESTING_MICROSOFT_CLIENT_ID!,
		)}&client_secret=${encodeURIComponent(
			process.env.DABBU_TESTING_MICROSOFT_CLIENT_SECRET!,
		)}&redirect_uri=${encodeURIComponent(
			process.env.DABBU_TESTING_MICROSOFT_REDIRECT_URI!,
		)}&refresh_token=${encodeURIComponent(
			process.env.DABBU_TESTING_MICROSOFT_REFRESH_TOKEN!,
	...
	process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN = `${
...
describe('test list request', () => {
	it('fail - no access token', async () => {
		const response = await request(app).get(
			'/files-api/v3/data/one-drive/%2F',
		)
	...
	it('fail - invalid access token', async () => {
		const response = await request(app)
			.get('/files-api/v3/data/one-drive/%2F')
	...
	it('fail - invalid path', async () => {
		const response = await request(app)
			.get('/files-api/v3/data/one-drive/%2Fthis-does-not-exist')
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
			)
	...
	it('fail - relative path', async () => {
		const response = await request(app)
			.get('/files-api/v3/data/one-drive/%2F..%2F.%2Ftests')
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
			)
	...
	it('succeed - with access token', async () => {
		const response = await request(app)
			.get('/files-api/v3/data/one-drive/%2Ftests%2Ftest-files')
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
			)
...
describe('test read request', () => {
	it('fail - no access token', async () => {
		const response = await request(app).get(
			'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fdocuments/Document.docx',
		)
	...
	it('fail - invalid path', async () => {
		const response = await request(app)
			.get(
				'/files-api/v3/data/one-drive/%2Ftest%2Ftest-files/non-existent-file',
			)
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
	...
	it('fail - relative path', async () => {
		const response = await request(app)
			.get('/files-api/v3/data/one-drive/%2Ftests%2F..%2F.%2F')
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
			)
...
describe('test create request', () => {
	it('fail - no access token', async () => {
		const response = await request(app).post(
			'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fuploads/Create%20Image%20Test.png',
		)
	...
	it('fail - relative path', async () => {
		const response = await request(app)
			.post(
				'/files-api/v3/data/one-drive/%2Ftests%2F..%2F.%2F/Create%20Image%20Test.png',
			)
			.attach('content', './tests/test-files/documents/Document.docx')
			.set(
				'Authorization',
	...
	it('succeed - upload and set lastModifiedTime', async () => {
		const response = await request(app)
			.post(
				'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fuploads/Last%20Modified%20Time%20Upload%20Image%20Test.png',
			)
			.attach('content', './tests/test-files/pictures/Image.png')
	...
	it('succeed - upload and set createdAtTime', async () => {
		const response = await request(app)
			.post(
				'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fuploads/Created%20At%20Time%20Upload%20Image%20Test.png',
			)
			.attach('content', './tests/test-files/pictures/Image.png')
...
describe('test update request', () => {
	...
	it('succeed - update path', async () => {
		const response = await request(app)
			.put(
				'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fuploads/Updated%20PDF.pdf',
			)
	...
	it('succeed - update lastModifiedTime', async () => {
		const response = await request(app)
			.put(
				'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fupdated/Updated%20PDF.pdf',
			)
			.field('lastModifiedTime', 'Thursday, 22 April 2021 06:27:05')
			.set(
				'Authorization',
...
describe('test delete request', () => {
	...
	it('succeed - delete folder', async () => {
		let response = await request(app)
			.delete(
				'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fuploads/',
			)
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
		...
		response = await request(app)
			.delete(
				'/files-api/v3/data/one-drive/%2Ftests%2Ftest-files%2Fupdated/',
			)
			.set(
				'Authorization',
				process.env.DABBU_TESTING_MICROSOFT_ACCESS_TOKEN!,
