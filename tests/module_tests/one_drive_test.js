/* Dabbu Files API Server - one_drive_test.js
...
test.before(async (t) => {
	...
	const server = await app(
		...
		['one_drive'] /* Enable only one_drive */
	)
...
test('making a request without authorization header should throw an error', async (t) => {
	...
	const listError = await t.throwsAsync(
		axios.get(`${t.context.apiUrl}/data/one_drive/%2Funknown-folder/`)
	)
	...
	const getError = await t.throwsAsync(
		axios.get(`${t.context.apiUrl}/data/one_drive/%2Funknown-folder/some-file`)
	)
	...
	const postError = await t.throwsAsync(
		axios.post(`${t.context.apiUrl}/data/one_drive/%2Funknown-folder/some-file`)
	)
	...
	const putError = await t.throwsAsync(
		axios.put(`${t.context.apiUrl}/data/one_drive/%2Funknown-folder/some-file`)
	)
	...
	const deleteError = await t.throwsAsync(
		axios.delete(
			`${t.context.apiUrl}/data/one_drive/%2Funknown-folder/some-file`
		)
	)
